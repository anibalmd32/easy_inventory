...
export function Cart() {
  ...
  const disabled = cart.items.length === 0 || !customer.dni || !customer.name || !customer.phone;
      ...
      <ShadCard.CardHeader>
        <div className="flex justify-between">
          ...
          <Button
            ...
            disabled={disabled}
            onClick={async () => await selectProductOperations.onGenerateInvoice()}
          >
            Generar factura
          </Button>
        </div>
        <ShadCard.CardDescription>Lista de los productos a facturar</ShadCard.CardDescription>
      </ShadCard.CardHeader>
        ...
        <ShadTable.Table>
              ...
              <ShadTable.TableRow key={i} className="hover:bg-gray-800/20">
                ...
                <ShadTable.TableCell className="text-right flex gap-2">
                  <Button
                    size={'icon'}
                    className="bg-gray-800 hover:bg-gray-800/20 transition-all duration-300 text-gray-200"
                    onClick={() => selectProductOperations.onSelectProductCounterDecrement(item.id)}
                  >
                    <Minus />
                  </Button>
                  <Button
                    size={'icon'}
                    className="bg-gray-800 hover:bg-gray-800/20 transition-all duration-300 text-gray-200"
                    onClick={() => selectProductOperations.onSelectProductCounterIncrement(item.id)}
                  >
                    <Plus />
                  </Button>
                  <Button
                    size={'icon'}
                    className="bg-gray-800 hover:bg-gray-800/20 transition-all duration-300 text-gray-200"
                    onClick={() => selectProductOperations.onUnselectProduct(item.id)}
                  >
                    <Trash />
                  </Button>
                </ShadTable.TableCell>
          ...
          <ShadTable.TableFooter className="bg-gray-900">
            <ShadTable.TableRow className="hover:bg-gray-800/20">
              ...
              <ShadTable.TableCell className="text-right">${cart.total}</ShadTable.TableCell>
            </ShadTable.TableRow>
